...
export default ({ state, navigation }) => {
    const { state:user } = useContext(UserContext);//o contexto de todas as informações do usuário

    //quando clica em um ícone, irá para a tela desse ícone
    ...
    return (
        <TabArea>
            ...
            <TabItem onPress={()=>goTo('Profile')}>
                {user.avatar != '' ? //se o avatar estiver preenchido, coloca um componente, que é o AvatarIcon
                    <AvatarIcon source={{uri: user.avatar}} />
                    : //se não tiver, coloca esse outro
